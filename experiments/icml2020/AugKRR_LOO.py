...
def main(cfg: DictConfig):
    ...
    if cfg.data.target_domain is not None:
        ...
        X_total = np.concatenate([X_tar_tr, X_tar_te])
        Y_total = np.concatenate([Y_tar_tr, Y_tar_te])

        for test_id in range(0, X_total.shape[0]):
            X_train = np.concatenate([X_total[:test_id], X_total[test_id+1:]])
            Y_train = np.concatenate([Y_total[:test_id], Y_total[test_id+1:]])
            X_test = X_total[test_id][:, None].T
            Y_test = Y_total[test_id][:, None]
            predictor.fit(X_train, Y_train, None, None, None)
            y_pred = predictor.predict(X_test)

            dicts = {**{
                    'AugKRR_' + name: metric(Y_test, y_pred)
            ...
            mmse = dicts['AugKRR_MSE']
            avg_mse += mmse

            # print("Target Domain:", data_run_id)
            # print(dicts)

        print("target domain", data_run_id, "avg_mse:", avg_mse/10)
        ...
        metrics: dict = {'MSE': mean_squared_error}
        print(X_tar_tr.shape, X_tar_te.shape)
        predictor = PartialLOOCVKRR()
        predictor.fit(X_tar_tr, Y_tar_tr, None, None, None)
